...
class Solution {
 public:
  int findCrossingTime(int n, int k, vector<vector<int>>& time) {
    ...
    int ans = 0;
    for (int i = 0; i < k; ++i) {
      left.push(make_pair(time[i][0] + time[i][2], i));
    }
    while (n) {
      ...
      while (pick.size() && pick.begin()->first <= ans) {
        ...
        right.push(make_pair(time[i][0] + time[i][2], i));
      ...
      if (right.size()) {
        ...
        right.pop();
        ans += time[i][2];
        put.insert(make_pair(ans + time[i][3], i));
        --n;
      } else if (left.size() && n > pick.size()) {
        int i = left.top().second;
        left.pop();
        ans += time[i][0];
        pick.insert(make_pair(ans + time[i][1], i));
      } else {
        int nextl =
